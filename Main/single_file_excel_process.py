...
class single_xlsx_processing:
    ...
    def __init__(self, df, id):
        ...
        nf = pp.requestDir + f'{id}\\' + df.split('\\')[-1]
        self.wb = openpyxl.load_workbook(df)
        self.sheet = self.wb.active

        # Output file path and extension for comma removed excel
        self.op_comma_removed_file = os.path.splitext(
            nf)[0] + "_comma_removed" + os.path.splitext(nf)[1]

        # Output file path and extension for processed excel
        self.opfile = os.path.splitext(
            nf)[0] + "_processed" + os.path.splitext(nf)[1]

        # Output file path and extension for processed CSV
        self.opcsvfile = os.path.splitext(nf)[0] + ".csv"
